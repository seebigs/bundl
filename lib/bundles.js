...
function add (targets, opts) {
    var bundl = this;
    var options = deepExtend({ outputDir: discoverRelativePath(new Error()) }, opts);
        ...
        if (bundl.CHAIN.length) {
            utils.each(bundl.CHAIN, function (plugin) {
                if (typeof plugin.all === 'function') {
                    extReturn = plugin.all.call(bundl, bundl.RESOURCES, Object.keys(bundl.CHANGEMAP), done);
    ...
    function eachDone (result) {
        ...
        if (typeof result === 'string') {
            ...
            utils.each(result.changemap, function (resourceName, modPath) {
                bundl.mapChanges(bundl, modPath, resourceName);
            });
            utils.each(result.sourcemap, function (smap) {
                console.log(smap);
            });
            moveAlong();

...
function setupWatcher(bundl, serverOptions) {
    if (serverOptions.rebuild !== 'never' && serverOptions.rebuild !== 'always') {
        ...
        // get ignored paths
        var ignored = ['\\.DS_Store','node_modules'];
        var gitIgnore = watchPath + '/.gitignore';
        if (fs.existsSync(gitIgnore)) {
            ignored = [];
            var ignores = fs.readFileSync(gitIgnore, 'utf8').split('\n');
            each(ignores, function (toIgnore) {
                if (toIgnore) {
                    ignored.push(toIgnore.replace(/[.*]/g, '\\$&'));
                }
            });
        }
        ...
        watch(watchPath, {
            ...
            filter: function (name) {
                return !new RegExp(ignored.join('|')).test(name);
            }

...
function startServer (bundl, serverOptions) {
    ...
    var app = express();
    var rootPath = serverOptions.rootPath ? path.resolve(serverOptions.rootPath) : process.cwd();
    ...
    app.use(bundlRouter(bundl, rootPath, serverOptions));
...
function bundlWebServer (bundl, serverOptions) {
    serverOptions = serverOptions || {};
    serverOptions.rebuild = serverOptions.rebuild || 'changed';
    ...
    function prepareStartServer () {
        if (serverOptions.rebuild === 'changed') {
            ...
            watch(watchPath, function(filepath) {
                var r, changed = bundl.getChangeMap(filepath);
                utils.each(changed, function (v, bundlePath) {
                    r = bundl.getResources()[bundlePath];
                    if (r) {
                        r.changed = true;
                    }
                });

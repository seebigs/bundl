...
function resolveTargets (bundl, targets, options) {
    ...
    var globOptions = { cwd: options.outputDir };

    if (typeof targets === 'string') {
        targetsMap[targets] = path.isAbsolute(targets) ? [targets] : matchByGlob(targets, globOptions, bundl);
        ...
        utils.each(targets, function (srcArray, name) {
            if (typeof srcArray === 'string') {
                targets[name] = path.isAbsolute(srcArray) ? [srcArray] : matchByGlob(srcArray, globOptions, bundl);
    ...
    utils.each(targetsMap, function (srcArray, name) {
        if (name) {
            destPath = path.resolve( path.join(options.outputDir || 'bundled', name) );
            ...
            srcArray.forEach(function (target) {
                if(target) {
                    if (typeof target === 'string') {
                        srcPath = path.isAbsolute(target) ? target : path.resolve( path.join(options.outputDir, target) );

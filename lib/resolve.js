...
function matchByGlob (match, relativeTo, globOptions, b) {
    if (!match) {
...
function targetsArrayToFiles (targets, relativeTo, globOptions, b) {
    var ignores = [];
    var matches = [];
    var files = [];
    ...
    matches.forEach(function (match) {
        files = files.concat(matchByGlob(match, relativeTo, globOptions, b));
    });
...
function resolveTargets (bundl, targets, options, relativeTo) {
    ...
    if (Array.isArray(targets)) {
        targetsArrayToFiles(targets, relativeTo, globOptions, bundl).forEach(function (file) {
        ...
        utils.each(targets, function (value, name) {
            ...
            if (Array.isArray(value)) {
                targets[name] = targetsArrayToFiles(value, relativeTo, globOptions, bundl);
            }
        });

...
function runTask (taskName, callbackQueue) {
    ...
    if (typeof task === 'function') {
        ...
        if (args.verbose && taskName !== 'default') {
            log('`' + taskName + '` is not a valid task.');
        }
    }
}
...
function run (taskName, callback) {
    ...
    setTimeout(function () {
        runTask(taskName, callbackQueue);
    }, 0);
